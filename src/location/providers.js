...
export const browserFetch = (...args) => globalThis.fetch(...args);
...
export class GooglePlacesProvider {
  mode = 'google';
  label = 'Google Places';
  constructor({ apiKey, fetchImpl = browserFetch, fallbackProvider = new DevelopmentLocationProvider() } = {}) { this.apiKey = apiKey?.trim(); this.fetchRequest = (...args) => fetchImpl(...args); this.fallbackProvider = fallbackProvider; this.sessionToken = null; }
  ...
  async search(query) {
    ...
    let response; try { response = await this.fetchRequest('https://places.googleapis.com/v1/places:autocomplete', {
  ...
  async details(placeId) {
    ...
    const response = await this.fetchRequest(`https://places.googleapis.com/v1/places/${encodeURIComponent(placeId)}${suffix}`, { headers: { 'X-Goog-Api-Key': this.apiKey, 'X-Goog-FieldMask': 'id,displayName,formattedAddress,location,addressComponents' } });
  ...
  async reverseGeocode({ latitude, longitude }) {
    const response = await this.fetchRequest('https://places.googleapis.com/v1/places:searchNearby', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Go-Api-Key': this.apiKey, 'X-Goog-FieldMask': 'places.id' }, body: JSON.stringify({ includedTypes: ['locality'], maxResultCount: 1, locationRestriction: { circle: { center: { latitude, longitude }, radius: 50000 } } }) });
...
export function createLocationProvider({ apiKey = import.meta.env?.VITE_GOOGLE_PLACES_API_KEY, fetchImpl = browserFetch } = {}) { return hasGooglePlacesKey(apiKey) ? new GooglePlacesProvider({ apiKey, fetchImpl }) : new DevelopmentLocationProvider(); }
