...
export class RoutineRepository {
  ...
  async saveLocationProfile(input, { currentDate = todayIso(), warmCache = true } = {}) { await this.ensureInitialized(); const previous = await this.getLocationProfile(); const profile = createLocationProfile({ ...input, locationVersion: previous ? String(Number(previous.locationVersion || 0) + 1) : '1' }); await this.adapter.put(STORE_NAMES.profiles, profile); const warm = async () => { await this.prayerCache.invalidateFuture(currentDate); await this.prayerCache.warmCurrentAndNext(currentDate, profile); };
    if (warmCache) { this.cacheWarmPromise = warm().catch((error) => { this.logger.warn('Prayer cache warming failed after profile save; on-demand calculation remains available.', error); return []; }); await this.cacheWarmPromise; } return profile; }
