...
export class RoutineRepository {
  ...
  async performInitialization() {
    ...
    const settings = await this.adapter.get(STORE_NAMES.settings, 'default'); const profile = await this.adapter.get(STORE_NAMES.profiles, 'default');
    if (!settings) await this.adapter.put(STORE_NAMES.settings, createUserSettings({ onboardingChoice: profile ? 'predefined-existing' : null }));
    else if (settings.onboardingChoice === undefined) await this.adapter.put(STORE_NAMES.settings, { ...settings, prayerRoutineEnabled: settings.prayerRoutineEnabled !== false, onboardingChoice: profile ? 'predefined-existing' : null });
  ...
  async reconcileProtectedActivities() {
    ...
    const nap = await this.adapter.get(STORE_NAMES.activities, 'nap'); if (nap?.protected || nap?.system || nap?.prayerKey) await this.adapter.put(STORE_NAMES.activities, normalizeActivity({ ...nap, protected: false, system: false, prayerKey: null, timeSlots: nap.timeSlots.map((slot) => ({ ...slot, prayerKey: null })) }));
  }
  ...
  async getActivities({ includeDeleted = false, includeDisabled = true, includePrayerDisabled = false } = {}) { await this.ensureInitialized(); const settings = await this.getSettings(); return (await this.adapter.getAll(STORE_NAMES.activities)).map(normalizeActivity).filter((item) => (includeDeleted || !item.deletedAt) && (includeDisabled || item.enabled) && (includePrayerDisabled || settings.prayerRoutineEnabled !== false || !isProtectedPrayerActivity(item))).sort((a, b) => a.order - b.order); }
  async setOnboardingChoice(choice) { if (!['predefined', 'custom'].includes(choice)) throw new Error('Choose a valid routine option.'); await this.ensureInitialized(); if (choice === 'predefined') { const existing = new Map((await this.getActivities({ includeDeleted: true, includePrayerDisabled: true })).map((item) => [item.id, item])); for (const item of DEFAULT_ACTIVITIES) if (!existing.has(item.id)) await this.adapter.put(STORE_NAMES.activities, clone(item)); } else { for (const item of await this.getActivities({ includeDeleted: true, includePrayerDisabled: true })) if (!isProtectedPrayerActivity(item)) await this.adapter.put(STORE_NAMES.activities, { ...item, enabled: false, deletedAt: item.deletedAt || now(), updatedAt: now() }); } return this.updateSettings({ onboardingChoice: choice }); }
  async setPrayerRoutineEnabled(enabled) { const value = Boolean(enabled); await this.updateSettings({ prayerRoutineEnabled: value }); return value; }
